#
# python train_grid_world_cpp_rshaping.py <train|test|run|curriculum> dim obstacles max_steps total_timesteps
#
...
if sys.argv[1] not in ['train', 'test', 'run', 'curriculum']:
    print("Usage: python train_grid_world_cpp_rshaping.py <train|test|run|curriculum> dim obstacles max_steps total_timesteps")
    sys.exit(1)
elif sys.argv[1] in ['train','curriculum']:
    if len(sys.argv) != 6:
        print("Usage for training: python train_grid_world_cpp_rshaping.py train|curriculum dim obstacles max_steps total_timesteps")
        sys.exit(1)
elif sys.argv[1] in ['test', 'run']:
    if len(sys.argv) != 4:
        print("Usage for testing/running: python train_grid_world_cpp_rshaping.py test|run dim obstacles")
        sys.exit(1)
...
if mode == 'train':
    ...
    log_dir = f'log/ppo_cpp_rshaping_{DIM}_{OBSTACLES}_{MAX_STEPS}_{ENTROPY_COEF}_{timestamp}'
    model_path = f'data/ppo_cpp_rshaping_{DIM}_{OBSTACLES}_{MAX_STEPS}_{ENTROPY_COEF}_{timestamp}.zip'
    ...
    model_name = input("Enter model filename (e.g., ppo_cpp_rshaping_5_3_200_0.05_20260324_100000): ")
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = f'log/ppo_cpp_rshaping_{DIM}_{OBSTACLES}_{MAX_STEPS}_{ENTROPY_COEF}_{timestamp}_curriculum'
    model_path = f'data/ppo_cpp_rshaping_{DIM}_{OBSTACLES}_{MAX_STEPS}_{ENTROPY_COEF}_{timestamp}_curriculum.zip'
