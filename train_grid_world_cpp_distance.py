#
# python train_grid_world_cpp_distance.py <train|test|run|curriculum> dim obstacles max_steps total_timesteps
#
...
try:
    gym.register(
        id="gymnasium_env/GridWorldCPPDistance-v0",
...
if mode == 'train':
    ...
    env = gym.make(
        "gymnasium_env/GridWorldCPPDistance-v0",
        size=DIM,
    ...
    model_path = f'data/ppo_cpp_distance_{DIM}_{OBSTACLES}_{MAX_STEPS}_{ENTROPY_COEF}_{timestamp}.zip'
    ...
    model_name = input("Enter model filename (e.g., ppo_cpp_distance_5_3_200_0.05_20260324_100000): ")
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = f'log/ppo_cpp_distance_{DIM}_{OBSTACLES}_{MAX_STEPS}_{ENTROPY_COEF}_{timestamp}_curriculum'
    model_path = f'data/ppo_cpp_distance_{DIM}_{OBSTACLES}_{MAX_STEPS}_{ENTROPY_COEF}_{timestamp}_curriculum.zip'
